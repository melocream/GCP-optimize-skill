...
description: >
  ...
  Cloud Run, Artifact Registry, BigQuery, Cloud Build, Logging 등
...
python3 --version
# Python 3.9 이하면: CLOUDSDK_PYTHON=/path/to/python3.12 을 gcloud 앞에 붙여서 실행
```
...
# Python 3.10+ 필요 시: CLOUDSDK_PYTHON=/path/to/python3.12 앞에 붙이기
...
-- ✅ JOIN 양쪽에 파티션 필터
FROM `ds.news` n
JOIN `ds.news_symbol_map` m ON n.news_id = m.news_id
WHERE n.created_at >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 24 HOUR)
  AND m.created_at >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 24 HOUR)
```
...
- **Gemini 관련**: Gemini Cloud Assist API(`cloudaicompanion`, `geminicloudassist`) 사용하지 않으면 비활성화
...
### 7-3. Firebase / Identity Platform
